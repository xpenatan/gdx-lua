...
public:
    ...
    const char* x_lua_tointeger(int index, size_t* len)     { return lua_tolstring(L, index, len); }
    lua_Number x_lua_tonumber(int index)                    { return lua_tonumber(L, index); }

    const char* x_lua_tostring(int index)                   { return lua_tostring(L, index); }

    int x_lua_type(int index)                               { return lua_type(L, index); }
    const char* x_lua_typename(int tp)                      { return lua_typename(L, tp); }
    ...
    const char* getOrString(const char* key, const char* otherwise) {
        sol::state_view lua(L);
        return lua.get_or(key, otherwise);
    }
